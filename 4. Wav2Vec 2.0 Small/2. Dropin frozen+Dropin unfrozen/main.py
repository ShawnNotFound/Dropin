...
def train_model(model, train_loader, dev_loader, num_epochs=10, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        # 验证阶段
        dev_eer = evaluate_model_eer(model, dev_loader, device)
...
def main():
    ...
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, collate_fn=collator, num_workers=4)
    dev_loader = DataLoader(dev_dataset, batch_size=32, shuffle=False, collate_fn=collator, num_workers=4)
    eval_loader = DataLoader(eval_dataset, batch_size=32, shuffle=False, collate_fn=collator, num_workers=4)
    ...
    for exp in experiments:
        ...
        if exp['name'] == 'baseline':
            save_path = '/data/yl7622/dropin/wav2vec2_small_dropin/saved_model/baseline_wav2vec2.pt'
            torch.save({
