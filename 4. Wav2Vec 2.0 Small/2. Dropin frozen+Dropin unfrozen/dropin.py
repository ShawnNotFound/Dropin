...
class ProjectedEncoderLayer(nn.Module):
    def __init__(self, origin_layer, config, dropin_dim = 768 * 2, output_dim=768):
        ...
        print(f'nn.linear({dropin_dim}, {output_dim})')
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        hidden_states2 = self.input_proj(hidden_states) 
        out2 = self.layer_addition(hidden_states2,
            attention_mask=attention_mask,
            output_attentions=output_attentions,)
...
class DropInWav2Vec2(nn.Module):
    ...
    def __init__(self,  
        ...
        self.base_model = Wav2Vec2Model(base_config)
            
        final_hidden_size = self.base_model.config.hidden_size
        
        if mode == 'frozen':
            
            # This is the Path to baseline model
            baseline_path = '/data/yl7622/dropin/wav2vec2_small_dropin/saved_model/baseline_model.pth'
    ...
    def _add_dropin_layers(self, freeze=True):
        ...
        for layer_idx, new_hidden_size in self.dropin_config.items():
            ...
            # 创建新层
            new_layer = ProjectedEncoderLayer(original_layer, dropin_layer, new_hidden_size, self.base_model.config.hidden_size)
