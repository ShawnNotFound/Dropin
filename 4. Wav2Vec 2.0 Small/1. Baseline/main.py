...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=1, save_dir="new_baseline_checkpoints"):
    """训练模型"""
...
def main():
    ...
    train_losses, val_losses, val_accuracies, val_eers = train_model(
        model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=45
    )
    
    # 绘制训练历史
    plot_training_history(train_losses, val_losses, val_accuracies, val_eers)
    
    # 加载最佳模型并在测试集上评估
    logger.info("Loading best model for final evaluation...")
    checkpoint = torch.load('new_baseline_checkpoints/best_model.pth', weights_only=False)
