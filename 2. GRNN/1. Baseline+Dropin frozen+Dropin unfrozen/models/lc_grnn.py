...
class GRNNLayer(nn.Module):
    ...
    def freeze_original_layers(self):
        """
        Freeze original parameters by setting requires_grad=False
        """
        if not self.dropin or self.dropin_size == 0:
            return
        
        self.is_frozen = True
        
        # Freeze all original layer parameters
        for name, param in self.named_parameters():
            if '_orig' in name:
    ...
    def get_param_stats(self):
        ...
        # Count original and dropin parameters
        original_params = sum(p.numel() for name, p in self.named_parameters() if '_orig' in name)
        dropin_params = sum(p.numel() for name, p in self.named_parameters() if 'dropin' in name)
        
        return {
            'total_params': total_params,
            'trainable_params': trainable_params,
            'original_params': original_params,
            'dropin_params': dropin_params
        }
...
class LCGRNN(nn.Module):
    ...
    # def freeze_original_layers(self):
    #     """Set requires_grad=False for original layers in all GRNN layers"""
    #     for layer in self.grnn_layers:
    #         layer.freeze_original_layers()
    def freeze_original_layers(self):
        for i, layer in enumerate(self.grnn_layers):
            if self.dropin_sizes[i] > 0:
                # Dropin layer：冻结原始部分
                layer.freeze_original_layers()
            else:
                # 没有 Dropin：整个层冻结
    ...
    def unfreeze_all_layers(self):
        """Unfreeze all layers"""
        for layer in self.grnn_layers:
            layer.unfreeze_all_layers()
    
    def get_param_stats(self):
        """Get parameter statistics for the whole model"""
        total_params = sum(p.numel() for p in self.parameters())
        trainable_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        
        # Get original/dropin param counts from GRNN layers
        original_params = 0
        dropin_params = 0
        
        for layer in self.grnn_layers:
            layer_stats = layer.get_param_stats()
            original_params += layer_stats['original_params']
            dropin_params += layer_stats['dropin_params']
        
        # Add SVM layer and LC extractor params
        other_params = sum(p.numel() for name, p in self.named_parameters() 
                         if not name.startswith('grnn_layers.'))
