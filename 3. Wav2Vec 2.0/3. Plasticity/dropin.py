...
class Wav2Vec2WithCustomLayer(nn.Module):
    def __init__(self, model_state_dict, pretrained_model_config="/data/yl7622/dropin/wav2vec/wav2vec2_model", num_labels=2):
        ...
        config = Wav2Vec2Config.from_pretrained(pretrained_model_config)
        ...
        layer5_dropin = ProjectedEncoderLayer(config, output_dim=1536)
        ...
        expand_and_load_encoder_layer(origin_layer5, layer5_dropin)
        self.wav2vec2.encoder.layers[5] = layer5_dropin  # 注意是索引5

        origin_layer6 = base_model.encoder.layers[6]
        expand_and_load_encoder_layer(origin_layer6, layer6_dropin)
        self.wav2vec2.encoder.layers[6] = layer6_dropin  # 注意是索引6
...
def train_model(ori_model, train_loader, val_loader, criterion, device, num_epochs=30, save_dir="checkpoints"):
    ...
    for epoch in range(num_epochs):
        ...
        # 保存最佳模型
        if val_eer <= best_eer and not math.isinf(eer_threshold):
            best_eer = val_eer
            best_epoch = epoch + 1
            torch.save({
                'epoch': epoch + 1,
...
def main():
    ...
    train_losses, val_losses, val_accuracies, val_eers = train_model(
        model, train_loader, val_loader, criterion, device, num_epochs=30, save_dir = '/data/yl7622/dropin/wav2vec/checkpoints_diffop_rerun'
    )
    
    # # 绘制训练历史
    plot_training_history(train_losses, val_losses, val_accuracies, val_eers, save_dir = '/data/yl7622/dropin/wav2vec/checkpoints_diffop_rerun')
    
    # 加载最佳模型并在测试集上评估
    logger.info("Loading best model for final evaluation...")
    checkpoint = torch.load('checkpoints_diffop_rerun/best_model.pth', weights_only=False)
    try:
        model.load_state_dict(checkpoint['model_state_dict'])
    except: # if the model is in dropin stage
